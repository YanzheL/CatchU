...
from third_party.mxnet_mtcnn.helper import nms, adjust_input, detect_first_stage_warpper
...
class MtcnnDetector(object):
    ...
    def find_tfrom_between_shapes(self, from_shape, to_shape):
        ...
        from_shape_points = from_shape.reshape(from_shape.shape[0] // 2, 2) # fixed by lyz
    ...
    def extract_image_chips(self, img, points, desired_size=256, padding=0):
        ...
        for p in points:
            shape = []
            for k in range(len(p) // 2):# fixed by lyz
                shape.append(p[k])
                shape.append(p[k + 5])
